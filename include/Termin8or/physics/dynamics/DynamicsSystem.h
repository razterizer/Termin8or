...
namespace t8x
{
  ...
  public:
    ...
    std::array<RigidBody*, NRB> add_rigid_bodies(const std::array<Sprite*, NRB>& sprites,
      ...
      std::function<std::vector<int>(int)> inertia_mats = [](int){ return std::vector<int> { 1 }; },
      std::function<std::vector<int>(int)> coll_mats = [](int){ return std::vector<int> { 1 }; })
    {
    ...
    void draw_dbg(ScreenHandler<NR, NC, CharT>& sh)
    {
      for (auto& rb : m_rigid_bodies)
      {
        ...
        sh.write_buffer("+", math::roundI(cm.r), math::roundI(cm.c), t8::Color16::Cyan);
      }
    }
  };
